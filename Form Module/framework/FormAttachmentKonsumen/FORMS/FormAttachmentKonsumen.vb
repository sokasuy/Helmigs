Public Class FormAttachmentKonsumen
    ...
    Private Structure consumen
        Dim nik As String
        Dim accountInstagram As String
        Dim nama As String
    End Structure
    ...
    Public Sub New(_dbType As String, _schemaTmp As String, _schemaPromotion As String, _connMain As Object, _username As String, _superuser As Boolean, _dtTableUserRights As DataTable, _addNewValues As String, _addNewFields As String, _addUpdateString As String, _groupData As String, _nik As String, _nama As String, _accountInstagram As String)
        Try
            ...
            With konsumen
                .nik = _nik
                .accountInstagram = _accountInstagram
                .nama = _nama
            End With
            strGroupData = _groupData

            If (konsumen.accountInstagram = "-") Then
                tbNIK.Text = konsumen.nik
            Else
                tbNIK.Text = konsumen.accountInstagram
            End If
            tbAccountInstagram.Text = konsumen.nama
            tbGroupData.Text = strGroupData
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "New FormAttachmentKonsumen Error")
        End Try
    End Sub

    Private Sub FormAttachmentKonsumen_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Call myCFormManipulation.CloseMyForm(Me.Owner, Me)
    End Sub

    Private Sub FormAttachmentKonsumen_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            isNew = True
            ...
            tableName = CONN_.schemaPromotion & ".attachmentkonsumen"

            isDataPrepared = True

            Dim pathFileSetting As String
            pathFileSetting = Application.StartupPath & "\SETTING.ini"
            fileAttachment.direktori = myCFileIO.ReadIniFile("PROGRAM_INFO", "CONSUMEN_ATTACHMENT", pathFileSetting)

            If (konsumen.accountInstagram <> "-") Then
                strTujuanFile = fileAttachment.direktori & "\" & konsumen.nik & "\" & konsumen.accountInstagram & "\"
            Else
                strTujuanFile = fileAttachment.direktori & "\" & konsumen.nik & "\"
            End If
            tbLokasiTujuanFile.Text = strTujuanFile

            myCFileIO.CreateDirectory(strTujuanFile)
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormAttachmentKonsumen_Load Error")
        End Try
    End Sub

    Private Sub FormAttachmentKonsumen_KeyDown(sender As Object, e As KeyEventArgs) Handles tbNIK.KeyDown, tbAccountInstagram.KeyDown, tbGroupData.KeyDown, tbLokasiAsalFile.KeyDown, tbLokasiTujuanFile.KeyDown, btnBrowse.KeyDown, tbNamaFile.KeyDown, tbExtensionFile.KeyDown, rtbKeterangan.KeyDown, btnSimpan.KeyDown, btnKeluar.KeyDown, btnAddNew.KeyDown, tbCari.KeyDown, btnTampilkan.KeyDown
        Try
            ...
            End If
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormAttachmentKonsumen_KeyDown Error")
        End Try
    End Sub
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, mKriteria As String, Optional gantiKriteria As Boolean = False)
        Try
            ...
            If (gantiKriteria) Then
                ...
                stSQL = "SELECT count(*) FROM " & tableName & " WHERE ((upper(" & mSelectedCriteria & ") LIKE '%" & mKriteria.ToUpper & "%') AND (nik='" & konsumen.nik & "') AND (accountInstagram='" & konsumen.accountInstagram & "' AND groupdata='" & strGroupData & "' OR ispublic='True'));"
            ...
            stSQL = "SELECT rid,nik,accountInstagram,nama,keterangan,pathtofile as path_to_file,namafile as nama_file,groupdata as group_data,ispublic as is_public,created_at,updated_at " &
                    "FROM ( " &
                        "SELECT sub.rid,sub.nik,sub.accountInstagram,sub.nama,sub.groupdata,sub.ispublic,sub.keterangan,sub.pathtofile,sub.namafile,sub.created_at,sub.updated_at " &
                        "FROM ( " &
                            "SELECT tbl.rid,tbl.nik,tbl.accountInstagram,tbl.nama,tbl.groupdata,tbl.ispublic,tbl.keterangan,tbl.pathtofile,tbl.namafile,tbl.created_at,tbl.updated_at " &
                            "FROM " & tableName & " as tbl " &
                            "WHERE ((upper(" & mSelectedCriteria & ") LIKE '%" & mKriteria.ToUpper & "%') AND (nik='" & konsumen.nik & "') AND (accountInstagram='" & konsumen.accountInstagram & "' AND groupdata='" & strGroupData & "' OR ispublic='True')) " &
            ...
            With dgvView
                ...
                .Columns("accountInstagram").Visible = False
            ...
            If (dgvView.RowCount > 0) Then
                ...
                ElseIf (e.ColumnIndex = dgvView.Columns("edit").Index) Then
                    ...
                    If Not IsDBNull(dgvView.CurrentRow.Cells("nik").Value) Then
                        If (dgvView.CurrentRow.Cells("accountInstagram").Value = "-") Then
                            tbNIK.Text = dgvView.CurrentRow.Cells("nik").Value
                        Else
                            tbNIK.Text = dgvView.CurrentRow.Cells("accountInstagram").Value
                        End If
                        tbAccountInstagram.Text = dgvView.CurrentRow.Cells("nama").Value
                        arrDefValues(1) = dgvView.CurrentRow.Cells("nik").Value
                        arrDefValues(2) = dgvView.CurrentRow.Cells("accountInstagram").Value
                    ...
                    If (Trim(tbLokasiAsalFile.Text).Length > 0) Then
                        ...
                        If Not isExist Then
                            If (myCFileIO.CopyFile(tbLokasiAsalFile.Text, tbLokasiTujuanFile.Text)) Then
                                ...
                                newValues = "'" & myCStringManipulation.SafeSqlLiteral(konsumen.nik) & "','" & myCStringManipulation.SafeSqlLiteral(konsumen.accountInstagram) & "','" & myCStringManipulation.SafeSqlLiteral(konsumen.nama) & "','" & myCStringManipulation.SafeSqlLiteral(tbGroupData.Text) & "','" & cbIsPublic.Checked & "','" & myCStringManipulation.SafeSqlLiteral(tbLokasiTujuanFile.Text) & "','" & myCStringManipulation.SafeSqlLiteral(tbNamaFile.Text) & "'," & ADD_INFO_.newValues
                                newFields = "nik,accountInstagram,nama,groupdata,ispublic,pathtofile,namafile," & ADD_INFO_.newFields
                                ...
                                Call myCShowMessage.ShowSavedMsg("Informasi file " & Trim(tbNamaFile.Text) & " dari karyawan " & Trim(tbAccountInstagram.Text) & " pada group data " & Trim(tbGroupData.Text))
                    ...
                    If Not IsNothing(updateString) Then
                        If (fileOperation) Then
                            If (copyOperation) Or (renameOperation) Then
                                ...
                                Call myCShowMessage.ShowUpdatedMsg("Informasi file " & Trim(tbNamaFile.Text) & " dari karyawan " & Trim(tbAccountInstagram.Text) & " pada group data " & Trim(tbGroupData.Text))
