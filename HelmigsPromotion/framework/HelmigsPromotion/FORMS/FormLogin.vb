Public Class FormLogin
    ...
    Public Sub New()
        ...
        Try
            Call myCStartup.StartUp("MAIN", COMPANY_.name, PROGRAM_.name, PROGRAM_.type, CONN_.dbMain, CONN_.dbLokal, CONN_.dbType, CONN_.schemaTmp, CONN_.schemaPromotion)
    ...
    Private Sub btnMasuk_Click(sender As Object, e As EventArgs) Handles btnMasuk.Click
        Try
            If (Trim(tbUserID.Text).Length > 0 And Trim(tbPassword.Text).Length > 0) Then
                ...
                isExist = myCDBOperation.IsExistRecords(CONN_.dbMain, CONN_.comm, CONN_.reader, "userid", CONN_.schemaPromotion & ".msuser", "userid='" & myCStringManipulation.SafeSqlLiteral(tbUserID.Text) & "'and passwd='" & myCStringManipulation.GetSHA1Hash(tbPassword.Text) & "' and block='False'")

                If (isExist) Then
                    USER_.username = Trim(tbUserID.Text)
                    stSQL = "SELECT superuser,lokasi FROM " & CONN_.schemaPromotion & ".msuser WHERE userid='" & myCStringManipulation.SafeSqlLiteral(tbUserID.Text) & "'and passwd='" & myCStringManipulation.GetSHA1Hash(tbPassword.Text) & "'"
